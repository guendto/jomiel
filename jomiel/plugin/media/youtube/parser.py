...
class Parser(PluginMediaParser):
    ...
    def add_streams(self, video_info):  # pylint: disable=too-many-locals
        ...
        video_spec = namedtuple('video_spec',
        ...
        fail_if_no_streams()
        encoded_stream_map = '%s,%s' % (stream_map, adaptive_fmts)
        ...
        def parse_fmt_list():
            ...
            def add_spec():
                ...
                if len(result) == 2:
                    fmt_type = int(spec[0])
                    formats[fmt_type] = video_spec(resolution=spec[1],
                                                   height=int(
                                                       result[1]),
                                                   width=int(result[0]))
